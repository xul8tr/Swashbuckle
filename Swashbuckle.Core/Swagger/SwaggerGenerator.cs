...
namespace Swashbuckle.Swagger
{
	public class SwaggerGenerator : ISwaggerProvider
	{
		...
		private readonly IDictionary<string, Info> _swaggerDocs;
		private readonly SwaggerGeneratorOptions _options;

		public SwaggerGenerator(
			IApiExplorer apiExplorer,
			JsonSerializerSettings jsonSerializerSettings,
			IDictionary<string, Info> swaggerDocs,
		...
		public SwaggerDocument GetSwagger(string rootUrl, string name)
		{
			...
			_swaggerDocs.TryGetValue(name, out Info info);
			if (info == null)
				throw new UnknownDocumentException(name);

			var paths = GetApiDescriptionsFor(name, info)
		...
		private IEnumerable<ApiDescription> GetApiDescriptionsFor(string name, Info info)
		{
			return (_options.DocInclusionPredicate == null)
				? _apiExplorer.ApiDescriptions
				: _apiExplorer.ApiDescriptions.Where(apiDesc => _options.DocInclusionPredicate(name, info, apiDesc));
		}
